...
│     └─ 没匹配 ↓                                              │
│                                                              │
│  第 2 步：lookup_examples  ← "历史上有类似案例吗？"            │
│     ├─ 找到 → 作为 Few-Shot 参考                              │
│     └─ 没找到 ↓                                              │
│                                                              │
...
1. 从 SQLite 查出该数据源的所有查询记录
2. 过滤：只取 `is_verified=1` 或 `success_count>=3` 的高质量案例
3. 打分排序（关键词匹配 ×2 + 表名匹配 ×1 + 认证加分 ×3 + 成功次数）
4. 返回 Top 3
...
| `lookup_examples` | 历史查询 | SQLite | `WHERE datasource_id=?` 全取，内存评分 |
...
| 查询示例 | `datanova.db` → `query_examples` | 自动保存 + 手动 | `lookup_examples` |
| 表查询示例 | `datanova.db` → `table_query_examples` | 手动维护 | `discover_schema` |
| Skills | `data/skills/*/SKILL.md` | 用户手动建文件 | `buildDataNovaSystemPrompt` |
| 注解 Skills | `data/annotations/*/SKILL.md` | 程序自动生成 | `buildDataNovaSystemPrompt` |
...
harness-factory.ts → createHarness()
  ...
  ├─ 加载 Skills
  │    loadAllSkills()
  │      → listSkillFiles()  ← 扫描 data/skills/ 和 data/annotations/

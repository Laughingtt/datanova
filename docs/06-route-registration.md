...
⑩ syncQueryExamplesFromHistory() — 对每个已启用数据源同步查询示例（启动时）
```
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/api/datasources/:dsId/query-history` | 按数据源查询历史（支持 `?limit=N`） |
| GET | `/api/query-history` | 全部查询历史（支持 `?limit=N`） |
...
> 仅在非生产环境可用（`NODE_ENV !== "production"` 时返回 403）。

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/test/datasources` | 直接创建数据源（跳过连接测试） |
| POST | `/api/test/query-history` | 直接创建查询历史记录 |
| POST | `/api/test/sync-examples/:datasourceId` | 手动触发查询示例同步 |
| GET | `/api/test/execution-stats/:datasourceId` | 获取查询执行统计 |
| GET | `/api/test/recent-sql-context/:datasourceId` | 获取最近 SQL 上下文（支持 `?limit=N`） |
| GET | `/api/test/auto-examples/:datasourceId` | 列出自动同步的查询示例 |
...
> **变更说明**：以前注解变更会调用 `generateAnnotationSkill()` 生成 `data/annotations/*/SKILL.md` 文件。现在已移除此路径，注解仅通过 `discover_schema` 工具的结构化格式注入，避免同一份标注在 prompt 中出现两次。
